...
Encoded polylines are used by Google Maps to draw lines and polygons, and are therefore supported in the [googleway](https://cran.r-project.org/web/packages/googleway/index.html) package. 

I am intending to update `googleway` to support plotting `sf` objects using encoded polylines.


## Caution

The word **lossy** is important to keep in mind, as the encoding process could reduce precision of your data. If you are after highly-accurate coordiantes this process probably isn't for you. 

However, if you need to reduce the size of objects/data, and want quicker plots (see the **Benchmarking** section), then this could help.
...
The two functions `polyline_wkt` and `wkt_polylne` can be used to convert `sfencoded` ojbects to and from well-known text.
...
I've provided these functions to enable the conversion of encoded polylines into other geometry foramts, shoudl they be required by other packages.  

## Bencmarking

You will note that the encoded strings are different between the two objects. For example

```{r}
enc[1, 'geometry'][[1]] == enc2[1, 'geometry'][[1]]
```

This is manifested through the **lossy**-ness of the encoding. However, the general shape of the information is preserved. These two maps are plots of the `enc` and `enc2` objects respectively
...
google_map(key = mapKey) %>%
  add_polygons(data = enc, polyline = "geometry", fill_colour = "#00FF00", fill_opacity = 0.2)
